...
function Player:init(map, name, side, range)
    ...
    self.sounds = {}
    self.states = {
        'idle', 'attacking', 'winning', 'special', 'dying', 'walking', 'hurt', 'jumping'
    }
    for i=1, #self.states do
        self.sounds[self.states[i]] = love.audio.newSource('sounds/' .. self.name .. '/' .. self.states[i] .. '.wav', 'static')
    end
end
...
function Player:update(dt)
    ...
    if self.state ~= 'duck' and self.state ~= 'waiting' then
